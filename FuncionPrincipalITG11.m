...
clear
close all
% mi sistema representa un alimentador de distribucion ITG 11
%% Parametros de control del AG
N=3;                      %Cantidad maxima de reconectadores
Npob=24;                  %cantidad de cromosomas a ser analizado
...
precio_un = 14730;     % Costo de Inversión un reconectador
Ce = 400;                 % Costo de energía Gs/kWh 13190 o 400
Nro_max_acoples=3;      %Cantidad maxima de acoplamientos que se desea encontrar en el alimentador (Max 3)
recos=[11 18 20]; %Insertar posiciones donde ya hay reconectadores dentro de la troncal

%% Extraccion de datos del excel
Ady=readmatrix('ITG11.xlsx','sheet','Grafos','Range','B3:BA54') %Matriz de Adyacencia
tasa_falla=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','F7:F58') %vector que contiene los lambda "i"
tiempo_reparo=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','H7:H58') % vector que contiene los tiempo de reparo
tiempo_seccionamiento=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','G7:G58') % vector que contiene los tiempo de seccionamiento
longitudes_tramos=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','C7:C58') % vector que contiene las longitudes de tramos
Li=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','J7:J58')  % Potencia Li (kW)
pesos_tramos=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','K7:K58')
dispositivos=readmatrix('ITG11.xlsx','sheet','Datos Iniciales','Range','M7:M58')    %Indica los tramos con IC, SC y SF del alimentador
acoplamientos=readcell('ITG11.xlsx','sheet','Datos Iniciales','Range','N7:P58')    %Indica los puntos de acoplamientos y cantidad de acoplamientos en cada tramo
...
%% Crear el digrafo
nombres_tramos=nombres_tramos(Ady);
...
for k=1:5
    ...
    colores(Ntr-cant_ramales+1:Ntr, :) = repmat([1 0 0], length(Ntr-cant_ramales+1:Ntr), 1); % Asignar un color diferente (rojo) a los nodos específicos
   
    m=find(mejores_5_FO{1,1}(k,:)); %localizamos la posicion de las LlT
    m(1)=[];
    colores(m, :) = repmat([1 1 0], length(m), 1);  %Las marcamos
...
figure(10);
colores = repmat([0 0 1], Ntr, 1); % Inicializar todos los nodos con un color predeterminado (azul)
colores(Ntr-cant_ramales+1:Ntr, :) = repmat([1 0 0], length(Ntr-cant_ramales+1:Ntr), 1); % Asignar un color diferente (rojo) a los nodos con fusibles
m1=find(cromo_arbi);    %localizamos la posicion de las LlT
m1(1)=[];               %eliminamos el interruptor de cabecera
colores(m1, :) = repmat([1 1 0], length(m1), 1);  %Las marcamos
plot(grafo1,'NodeColor',colores,'EdgeColor','g');
